/* ============================================
   SDTM Automation Academy - Main Application
   ============================================ */
...
const App = {
    ...
    init() {
        ...
        if (localStorage.getItem('sdtm-automation-darkmode') === 'true') {
    ...
    loadProgress() {
        try {
            const saved = localStorage.getItem('sdtm-automation-progress');
            if (saved) this.progress = JSON.parse(saved);
            const savedQuiz = localStorage.getItem('sdtm-automation-quiz-results');
    ...
    saveProgress() {
        localStorage.setItem('sdtm-automation-progress', JSON.stringify(this.progress));
        localStorage.setItem('sdtm-automation-quiz-results', JSON.stringify(this.quizResults));
    },
        ...
        let html = `
            <div class="fade-in">
                <div class="dashboard-hero">
                    <h2>SDTM Automation Academy へようこそ</h2>
                    <p>SDTMデータセット作成の自動化手法を基礎から実践まで学べるプラットフォームです。</p>
                </div>
    ...
    toggleDarkMode() {
        ...
        if (isDark) {
            ...
            localStorage.setItem('sdtm-automation-darkmode', 'false');
        } else {
            document.documentElement.setAttribute('data-theme', 'dark');
            localStorage.setItem('sdtm-automation-darkmode', 'true');
        }
    },
    ...
    resetProgress() {
        if (confirm('全ての学習進捗をリセットしますか？この操作は元に戻せません。')) {
            this.progress = {};
            this.quizResults = {};
            localStorage.removeItem('sdtm-automation-progress');
            localStorage.removeItem('sdtm-automation-quiz-results');
            this.buildSidebar();
            this.updateGlobalProgress();
            this.showDashboard();
        }
